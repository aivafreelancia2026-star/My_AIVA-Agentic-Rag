...
def _notebook_chat(notebook_id: str, user_message: str, context_manager, llm, data: dict):
    ...
    if not context_str:
        no_ctx = (
            f'The notebook "{agent["name"]}" has no indexed content yet. '
            'Please add sources and wait for indexing to complete.'
        )
        if stream_requested:
            def _empty():
                yield f"data: {json.dumps({'type': 'token', 'content': no_ctx})}\n\n"
                yield f"data: {json.dumps({'type': 'done', 'sources': [], 'processing_time': 0.0, 'images': [], 'blocks': [{'type':'text','content':no_ctx}]})}\n\n"
            return Response(stream_with_context(_empty()), headers={
                'Cache-Control': 'no-cache', 'Content-Type': 'text/event-stream', 'X-Accel-Buffering': 'no'
            })
        return jsonify({
            'response': no_ctx,
            'query_type': 'notebook_empty',
    ...
    formatted_sources = [
        {'source_document': m['source'], 'document_title': m['agent_name'], 'relevance_score': m['score']}
        for m in meta_list
    ]

    if llm is None:
        msg = 'LLM not available. Context found:\n\n' + context_str[:800]
        return jsonify({'response': msg, 'query_type': 'notebook', 'sources': formatted_sources,
                        'processing_time': 0.0, 'images': [], 'blocks': [{'type':'text','content':msg}]})
    ...
    if stream_requested:
        def _stream():
            try:
                ...
                context_manager.add_exchange(
                    question=user_message, answer=full_text,
                    sources=[], context_used='', query_type='notebook'
                )
                blocks = [{'type': 'text', 'content': p.strip()}
                          for p in full_text.split('\n\n') if p.strip()]
                yield f"data: {json.dumps({'type': 'done', 'sources': formatted_sources, 'processing_time': round(_time.time()-start,2), 'images': [], 'blocks': blocks})}\n\n"
            except Exception as e:
                logger.error(f"Notebook stream error: {e}\n{traceback.format_exc()}")
                yield f"data: {json.dumps({'type': 'error', 'message': _friendly_llm_error(e)})}\n\n"
                yield f"data: {json.dumps({'type': 'done', 'sources': [], 'processing_time': 0.0, 'images': [], 'blocks': []})}\n\n"
        return Response(stream_with_context(_stream()), headers={
    ...
    context_manager.add_exchange(question=user_message, answer=text,
                                  sources=[], context_used='', query_type='notebook')
    ...
    return jsonify({
        ...
        'query_type': 'notebook',
        'sources': formatted_sources,
        'processing_time': round(_time.time() - start, 2),
        'images': [],
        'blocks': blocks,
    })
...
def api_chat():
    ...
    try:
        ...
        return jsonify({'error': friendly, 'message': friendly}), 502
